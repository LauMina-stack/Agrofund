<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>AgriFund</title>
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">  

</head>
<body class="bg-gray-900 text-white">
    <div class="container mx-auto px-4 py-8">
        <!-- Header Section -->
        <div class="text-center">
            <h1 class="text-4xl font-bold">AgriFund</h1>
            <p class="mt-2 text-lg">Plateforme de financement participatif pour l'agriculture durable</p>
            <div class="mt-4">
                <button class="bg-green-500 text-white py-2 px-4 rounded-full mr-2"> <a href="dasbord investisseurs.php">Démarrer un projet</a></button>
                <button class="text-white py-2 px-4 rounded-full"><a href="enregistrement des investissseurs.php">Investir</a></button>
            </div>
        </div>

        <!-- Stats Section -->
        <div class="flex justify-center mt-12 space-x-8">
            <div class="text-center">
                <p class="text-3xl font-bold">€2.5M+</p>
                <p class="mt-2">Financements réalisés</p>
            </div>
            <div class="text-center">
                <p class="text-3xl font-bold">150+</p>
                <p class="mt-2">Projets financés</p>
            </div>
            <div class="text-center">
                <p class="text-3xl font-bold">5000+</p>
                <p class="mt-2">Investisseurs actifs</p>
            </div>
        </div>

        <!-- How It Works Section -->
        <div class="mt-16">
            <h2 class="text-center text-2xl font-bold">Comment ça fonctionne</h2>
            <div class="flex justify-center mt-8 space-x-8">
                <div class="bg-gray-800 p-6 rounded-lg text-center w-64">
                    <i class="fas fa-seedling text-green-500 text-3xl mb-4"></i>
                    <h3 class="text-xl font-bold">Créez votre projet</h3>
                    <p class="mt-2">Présentez votre projet agricole et définissez vos besoins de financement</p>
                </div>
                <div class="bg-gray-800 p-6 rounded-lg text-center w-64">
                    <i class="fas fa-check-circle text-green-500 text-3xl mb-4"></i>
                    <h3 class="text-xl font-bold">Validation experte</h3>
                    <p class="mt-2">Notre équipe vérifie et valide votre projet selon des critères stricts</p>
                </div>
                <div class="bg-gray-800 p-6 rounded-lg text-center w-64">
                    <i class="fas fa-hand-holding-usd text-green-500 text-3xl mb-4"></i>
                    <h3 class="text-xl font-bold">Collecte de fonds</h3>
                    <p class="mt-2">Les investisseurs soutiennent votre projet en toute transparence</p>
                </div>
                <div class="bg-gray-800 p-6 rounded-lg text-center w-64">
                    <i class="fas fa-chart-line text-green-500 text-3xl mb-4"></i>
                    <h3 class="text-xl font-bold">Suivi du projet</h3>
                    <p class="mt-2">Partagez l'avancement et les résultats avec vos investisseurs</p>
                </div>
            </div>
        </div>

        <!-- Call to Action Section -->
        <div class="bg-green-500 mt-16 py-12 text-center">
            <h2 class="text-2xl font-bold">Prêt à développer l'agriculture durable ?</h2>
            <p class="mt-2">Rejoignez notre communauté d'agriculteurs et d'investisseurs engagés</p>
            <button class="bg-gray-900 text-white py-2 px-4 rounded-full mt-4">Commencer maintenant</button>
        </div>
    </div>
</body>
</html>